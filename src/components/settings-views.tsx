...
export function AiSettings({ accessCode }: { accessCode: string }) {
  ...
  async function save(next = settings) {
    ...
    try {
      ...
      setSettings(next); setState("saved");
  ...
  return <main className="content-page settings-page"><div className="page-title-row"><div><h1>AI 客服設定</h1><p>AI 只產生草稿，確認後由您按下送出</p></div><button className="primary ai-save" onClick={() => void save()} disabled={!loaded || state === "saving"}><Save size={15}/>{state === "saving" ? "儲存中…" : "儲存設定"}</button></div>
    <div className={`settings-notice settings-state ${state}`} role="status">{state === "loading" ? "正在讀取設定…" : state === "saving" ? "正在儲存…" : state === "dirty" ? "有尚未儲存的修改" : state === "error" ? notice || "無法讀取設定，請重新開啟此頁" : <><Check size={14}/>設定已同步</>}</div>
    <section className="settings-card"><header><Bot/><div><h2>人工確認後送出</h2><p>AI 自動發送已移除。客服草稿需由您確認並按下送出；選單固定回覆與休息時間提醒不受影響。</p></div></header></section>
    <fieldset className="ai-settings-fields" disabled={!loaded || state === "saving"}>

    <section className="settings-card"><header><Sparkles/><div><h2>回覆語氣</h2><p>AI 可整理語句、帶入顧客稱呼與問題，但不自行承諾預約結果。</p></div></header><label className="field">使用模型<select value={settings.model} onChange={event => update({ model: event.target.value })}><option value="gpt-5-mini">gpt-5-mini（建議）</option><option value="gpt-5.1">gpt-5.1</option></select></label><label className="field">客服語氣<textarea value={settings.tone} onChange={event => update({ tone: event.target.value })}/></label></section>
    <section className="settings-card"><header><ShieldCheck/><div><h2>人工確認提醒</h2><p>產生草稿時提醒您確認以下問題</p></div></header>{defaultRules.map(item => <label className="check-row" key={item}><input type="checkbox" checked={settings.handoffRules.includes(item)} onChange={event => toggleRule(item, event.target.checked)}/>{item}</label>)}</section>
    </fieldset>
  </main>;
}
