...
const pageActionSeed: Record<MenuPage, RichAction[]> = {
  home: ["旅館位置", "活動專區", "IG 點我", "住宿價目表", "預約須知", "接送服務"].map((label, index) => ({ id: index + 1, label, type: "message", value: label })),
  service: ["商品賣場", "購買方式", "最新優惠", "配送說明", "商品問題", "聯絡客服"].map((label, index) => ({ id: index + 1, label, type: "message", value: label })),
};
